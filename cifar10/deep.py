...
model.add(Convolution2D(64, 3, nb_conv, nb_conv, border_mode='same'))    # 64 x 32 x 32
model.add(Activation('relu'))
model.add(Convolution2D(64, 64, nb_conv, nb_conv, border_mode='same'))  # 128 x 14 x 14
model.add(Activation('relu'))
...
model.add(Activation('relu'))
#model.add(Convolution2D(128, 128, nb_conv, nb_conv, border_mode='same'))  # 128 x 14 x 14
#model.add(Activation('relu'))
model.add(MaxPooling2D(poolsize=(nb_pool, nb_pool)))                     # 128  x 7  x 7
model.add(Dropout(0.3))

model.add(UpSample2D())                                                  # 128 x 14 x 14
#model.add(Convolution2D(128, 128, nb_conv, nb_conv, border_mode='same'))   # 64 x 14 x 14
#model.add(Activation('relu'))
model.add(Convolution2D(64, 128, nb_conv, nb_conv, border_mode='same'))   # 64 x 14 x 14
model.add(Activation('relu'))
model.add(UpSample2D())                                                  #  64 x 28 x 28
model.add(Convolution2D(64, 64, nb_conv, nb_conv, border_mode='same'))   # 64 x 14 x 14
model.add(Activation('relu'))
model.add(Convolution2D(3, 64, nb_conv, nb_conv, border_mode='same'))    #   1 x 28 x 28
